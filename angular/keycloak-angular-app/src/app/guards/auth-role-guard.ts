...
const isAccessAllowed = async (
  ...
  const hasRequiredRole = (role: string): boolean => {
    const resourceHasRole = Object.values(grantedRoles.resourceRoles).some(
      (roles) => roles.includes(role)
    );
    const realmHasRole = grantedRoles.realmRoles?.includes(role);
    return resourceHasRole || realmHasRole;
  };
